...
const COOKIE_NAME = 'admin_mfa_ok';

/** Create a signed cookie value: userId:expiresAt:HMAC-SHA256 */
export function signAdminMfaCookie(userId: string, expiresAt: number): string {
...
/**
 * Validate the admin MFA cookie value.
 * Returns the userId string if the cookie is valid and not expired, null otherwise.
 */
export function verifyAdminMfaCookie(value: string): string | null {
  if (!value) return null;
  // Format: uuid:timestampMs:hexsig
  // UUID has hyphens (no colons), timestamp is digits, sig is hex — safe to split on ':'
...
export { COOKIE_NAME as ADMIN_MFA_COOKIE_NAME };
